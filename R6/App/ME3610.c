...
const GPRS_INIT_TAB_STR gMe3610InitTab[] = {
    {NULL, "OK", 3000, 0, ME3610_check_test},					//
    {"ATE0\r", "OK", 5000, 10, ME3610_check_ATE},				//
    {"AT+GMM\r","OK",1000,3,ME3610_check_GMM},
    #if (0 == TEST_AIR_WIRE)
    {"ATI\r", "OK", 3000, 10, ME3610_check_ATI},				//显示产品ID信息
    {"AT+CPIN?\r", "READY", 500, 10, ME3610_check_cpin},		//指示用户是否需要密码
    {"AT+ZGETICCID\r", "OK", 500, 10, ME3610_check_CCID},		//查询SIM卡的CCID
	{"AT+CREG?\r","OK", 1000, 50, ME3610_check_CREG},			//检查GSM网络注册状态
	{"AT+COPS?\r","OK",1000,50,ME3610_check_COPS},				//获取 网络状态
	{"AT+ZPAS?\r","OK",1000,50,ME3610_check_ZPAS},				//获取 网络状态
    {"AT+ZIPCFG?\r","OK",2000,50,ME3610_check_ZIPCFG},
	{"AT+ZIPCALL?\r","OK",3000,50,ME3610_check_ZIPCALL},		//查看链接状态---如果链路是打开需要关闭后再连接
	{"AT+ZIPCALL=0\r","OK",5000,100,ME3610_check_ZIPCALL0},		//关闭链接
	{"AT+ZIPCALL=1\r","OK",5000,100,ME3610_check_ZIPCALL1},		//建立链路 获取IP地址
    {"AT+CSQ\r", "OK", 1000, 10, ME3610_check_CSQ},             //查看信号强度
    {"AT+ZIPSETRPT=1\r","OK",1000,10,ME3610_check_ZIPSETPRT},
	{NULL, NULL, 0, 0, ME3610_ipopen},
    #endif
};
...
void gprs_power_off(void)
{
	...
	//通过AT指令关机 关机时间15s
	for (int i=0; i<2; i++) {
        if (Me3610SendCmd("AT+ZTURNOFF\r", "OK", 1000, 0)==0) {
...
int ME3610GetModuleId(char *pStr)
{
    ...
    if(p)
    {
        ...
        if(strncmp(moduleId,"ME3610",strlen("ME3610")) == 0)
        {
            gGetModuleType = NET_MODE_4G_ME3610;
...
int Me3610CheckRes(char *cmd, char *res, uint16_t tmo)
{
    char *ret=NULL;
    uint8_t c;
    int cnt = 0;
        ...
        if (ret) {
            ...
            if (NULL != cmd) {
                if (strstr(cmd, "AT+CSQ")) {
                    ...
                    retv = ME3610GetCREG((void*)gprsBuffer);
                }
                else if(strstr(cmd,"AT+GMM")) {
                    retv = ME3610GetModuleId((void*)gprsBuffer);
                }
            }
            break;
        }
...
int Me3610SendCmd(char *cmd, char *ack, uint16_t waittime, int flag)
{
    ...
    if (CL_OK == Me3610CheckRes(cmd, ack, waittime)) {
...
int Me3610CheckResnospace(char * ack)
{
    uint8_t data;
    uint8_t flag = 0;
...
//flag = 1是发>
int Me3610SendCmdnospace(char * cmd, char * ack, int waitCnt, int waittime)
{
	int i;
    int k;
...
int ME3610CheckModeCmd(char ok, uint8_t retry, uint16_t delay)
{
    if (ok==0) {
        istage++;
        return CL_OK;
        ...
        gSimStatus |= (1<<istage);
        istage = ME3610_RESET; /* goto reset */
	}
    return CL_FAIL;
}
...
{
    CL_LOG("in.\n");
    return ME3610CheckModeCmd(ok, retry, 0);
}
...
{
    CL_LOG("reset ME3610 module\n");
    Me3610Reset();
    for (int i=0; i<2; i++) 
    {
        if (Me3610SendCmd("AT\r", "OK", gMe3610InitTab[ME3610_RESET].wait, 0)==0) 
        {
            CL_LOG("4G Test ok ...\n");
            istage++;
            return CL_OK;
...
int Me3610OpenSet(int socket, char* addr, int port)
{
    char cmd_req[64] = {0};

    sprintf(cmd_req,"AT+ZIPOPEN=%d,0,%s,%d\r", socket, addr, port);
    if(!Me3610SendCmd(cmd_req, "OK", 5000, 0)) {
...
int ME3610_ipopen(char ok, uint8_t retry)
{
    ...
    switch(state) {
        case NET_STATE_SOCKET0_OPEN:
        {
            ...
            for(int i = 0; i < 10; i++)
            {
                res = Me3610OpenSet(1, system_info.server_ip, system_info.server_port);
                if(!res)
                {
                    ready = CL_TRUE;
                    break;
...
//返回 0成功
int Me3610SendData(char *data, int len, char *ack, uint16_t waittime)
{
	int res = 1;

    UsartSend(GPRS_UART_PORT, (void*)data, len);
    #if (1 == ME3610_DEBUG)
	PrintfData("Me3610", (void*)data, len);
    #endif
    if ((ack == NULL) || (waittime == 0)) return CL_OK;
    if (CL_OK == Me3610CheckRes(NULL, ack, waittime)) {
...
int Me3610SocketSend(int socket, uint8_t* data, uint16_t len)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        res = Me3610SendCmd(cmd_req, ">", 1000, 1);
        if (0 != res) {
            CL_LOG("call Me3610SendCmd=%d,error.\n",res);
        }

        if (CL_OK == (res = Me3610SendData((char*)data, len, "OK", 10000))) {
            break;
        }else{
    ...
    if (res == 0) {
        ...
        if (system_info.tcp_tx_error_times >= TX_FAIL_MAX_CNT) {
            ...
            gSimStatus |= (1<<(ME3610_STATE_NUM+1));
        }
    }
    return res;
}
...
int Me3610FtpGet(const char* serv, const char* un, const char* pw, const char* file)
{
	...
	gprsBuffer[0] = 0;
    Me3610SendCmd("AT$ZFTPCLOSE\r","OK",1000,0);	//AT$ZFTPCLOSE, OK
	Me3610SendCmd("AT+ZIPCLOSE=1\r","OK",1000,0);	//AT+ZIPCLOSE=1, OK
	Me3610SendCmd("AT+CSQ\r","OK",1000,0);
	Me3610SendCmd("AT+ZPAS?\r","OK",1000,0);
	Me3610SendCmd("AT+CGDCONT=1,\"IP\",\"ctnet\"\r","OK",1000,0); //只处理电信网络
	Me3610SendCmd("AT$ZPDPACT=1\r","OK",1000,0);
	...
	if (Me3610SendCmdnospace(ipconfig, "FTPSIZE:",50,100) == 0) {//AT$ZFTPSIZE=TMP/T.LOG
		sprintf(tmp,"FTPSIZE:");
	...
	Me3610SendCmd("AT+ZIPCLOSE=1\r","OK",1000,0);	//AT+ZIPCLOSE=1, OK
	Me3610SendCmd("AT+ZIPCALL=0\r","OK",1000,0);	//AT+ZIPCLOSE=1, OK
	Me3610SendCmd("AT+GMR\r","OK",1000,0);	//AT+ZIPCLOSE=1, OK
    //Me3610SendCmd("AT+ZCDRUN?\r","OK",1000,0);	//AT+ZIPCLOSE=1, OK
    //Me3610SendCmd("AT$ZFTPCLOSE\r","OK",1000,0);	//AT+ZIPCLOSE=1, OK
    //Me3610SendCmd("AT+ZIPSETRPT=?\r","OK",1000,0);	//AT+ZIPSETRPT=0, OK
    //Me3610SendCmd("AT+ZIPSETRPT=0\r","OK",1000,0);	//AT+ZIPSETRPT=0, OK
    //Me3610SendCmd("AT+ZIPSETRPT=?\r","OK",1000,0);	//AT+ZIPSETRPT=0, OK
    ...
    while(1) {
        Feed_WDT();
        if (120 < (uint32_t)(GetRtcCount() - over_time)) {
        ...
        if (Me3610SendCmdnospace(ipconfig,"\r\nOK\r\n",100, 50) == 0) {//AT$ZFTPGET=TMP/T.LOG
            offset += getLen;
            if (16 == getLen) {
                if (0 == getCnt) {
            ...
            if (cfize >= fsize) {
    			if (checksum == chsum_in) {
    				CL_LOG("write file size %d, checksum %4X,ftp get success finish! .\n", fsize, checksum);
...
int Me3610Reconnect(void)
{
	...
	while(1) {
        Feed_WDT();
        OS_DELAY_MS(gMe3610InitTab[istage].nwait*10);
        if (gMe3610InitTab[istage].cmd) {
            ok = Me3610SendCmd(gMe3610InitTab[istage].cmd,gMe3610InitTab[istage].res,gMe3610InitTab[istage].wait, 0);
        }

        if (gMe3610InitTab[istage].process) {
            if (CL_OK == gMe3610InitTab[istage].process(ok, retry)) {
                retry = 0;
            }else{
                retry++;
            }
		...
		if (istage == ME3610_STATE_NUM) {
			CL_LOG("me3610 init and set socket ok.\n");
			return CL_OK;
		}else if (ME3610_RESET == istage) {
            CL_LOG("me3610 init and set socket fail.\n");
            return CL_FAIL;
        }
	}
}
